...
async def run_weather_agent_no_approval(stream: bool) -> None:
    """Run the weather agent without requiring any human approval for tool calls."""
    print(f"\n=== Weather Agent (No Approval Required) ({'Streaming' if stream else 'Non-Streaming'}) ===\n")
    ...
    async with Agent(
        ...
        name="WeatherAgentNoApproval",
        instructions=("You are a helpful weather assistant. Use the get_weather tool to provide weather information."),
        tools=[get_weather],  # Only use the tool that never requires approval
    ) as agent:
        query = "Can you give me an update of the weather in LA and Portland?"
        print(f"User: {query}")
...
async def main() -> None:
    ...
    # await run_weather_agent_no_approval(stream=True)
    await run_weather_agent_no_approval(stream=False)
